...
namespace DeltaUserGateway.Sender
{
    public class SenderConnectionV2 : InternalCommsServerClient
    {
        ...
        public override async Task HandleMessage(int opcode, Dictionary<string, byte[]> payloads)
        {
            switch(opcode)
            {
                case 1: await SendGatewayMsg(payloads); break;
            }
        }

        private async Task SendGatewayMsg(Dictionary<string, byte[]> payloads)
        {
            ...
            //Distribute messages
            await SessionHolder.DistributeMessage(filter, payloads["DATA"]);
        }
    }
}
